...
class Maquina {
public:
    Maquina(const Digrafo& g, int sr, int de) :
        visit(g.V(), false), dist(g.V(), 0), s(sr), d(de) {
        busca(g, sr, de);
    }
...
private:
    ...
    void busca(const Digrafo& g, int s, int d) {
        queue<int> q;
        bool found = false;
        int v;
...
bool tratar_caso(const Digrafo& g) {
    int ini, fin;
    cin >> ini >> fin;
    if (!cin)
        return false;
