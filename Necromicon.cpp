...
class Necromicon {
public:
    Necromicon(const Digrafo& g) :
        visit(g.V(), false), fin(g.V()-1), apilado(g.V(), false), ciclo(false) {
        profunda(g, 0);
    }
    bool acaba() { return visit[fin]; }
    bool hayCiclo() { return ciclo; }
private:
    vector<bool>visit;
    vector<bool>apilado;
    int fin;
    ...
    void profunda(const Digrafo& g, int v) {
        visit[v] = true;
        apilado[v] = true;
        for (int w : g.ady(v)) {
            if (!visit[w])
...
bool tratar_caso() {
    ...
    Necromicon ne(g);
    if (!ne.acaba()) cout << "NUNCA\n";
    else if (!ne.hayCiclo()) cout << "SIEMPRE\n";
    else if(ne.acaba() && ne.hayCiclo()) cout << "A VECES\n";
    return true;
}
